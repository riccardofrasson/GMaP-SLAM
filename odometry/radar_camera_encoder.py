...
class OdometryPipeline(nn.Module):
    ...
    def __init__(
        ...
        if optimizer_params is None:
            optimizer_params = {}
        self.pose_optimizer = ClipWindowOptimizer(
            cost_volume_channels=cv_params["cost_ch"], **optimizer_params
        ).to(self.device)
    ...
    def _unpad_and_separate(self, painted_pc_padded, lengths):
        ...
        pc_unpadded = pc_painted_unpadded[:, :5]  # xyz, vr, rcs
        cam_feat_unpadded = pc_painted_unpadded[:, 5:]  # camera features
    ...
    def forward(self, batch, history=None):
        ...
        # --- Load the final transformation matrix from the batch ---
        ...
        # Regress pose using the optimizer
        rotation_q, translation_v, new_history = self.pose_optimizer(
            cost_volume=cost_volume_features, history=history
        )

        predicted_pose = torch.cat([translation_v, rotation_q], dim=1)

        return predicted_pose, new_history
