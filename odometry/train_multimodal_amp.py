...
def pose_7d_to_matrix(pose_vec):
    ...
    if norm < 1e-6:
        T = np.eye(4)
        T[:3, 3] = t  # Still apply the predicted translation
        return T
...
# ==============================================================================
# 2. TRAIN/VALIDATION LOOP (MODIFIED FOR FUSED MODEL AND AMP)
# ==============================================================================
def run_epoch(
    model,
    dataloader,
    ...
    for batch in pbar:
        ...
        history = None
        predicted_poses_for_validation = []

        # --- Loop to process the sequence step-by-step ---
        for t in range(clip_len_poses):
            ...
            with autocast(
                ...
                # Prepare the sub-batch for Radar-Camera features
                sub_batch = {
                    "radar_t0": radars[:, t, ...],
                    "radar_t1": radars[:, t + 1, ...],
                    "image_t0": images[:, t, ...],
                ...
                # Prepare the single IMU clip for this step
                imu_clip = imus[:, t, ...]
                imu_clip_4d = imu_clip.unsqueeze(1)  # Shape [B, 1, Max_IMU_Len, 6]
            ...
            if is_training:
                # AMP: Scale loss, backward pass, and optimizer step
        ...
        if not is_training:
            # Stack poses for the whole clip for validation metrics
...
def main():
    ...
    plot_save_dir = Path("training_FusedModel")
    ...
    for epoch in range(1, EPOCHS + 1):
        # Pass the scaler to the run_epoch function
        ...
        print(
            f"  Validation: Loss: {val_loss:.4f}, MSE_t: {val_t_err:.4f}, Err_q: {val_q_err:.2f}°, ATE: {val_ate:.4f} m, Drift: {val_rot_drift:.4f} °/m"
        )
        ...
        if epoch > 1:
            # Plot history at the end of each epoch
